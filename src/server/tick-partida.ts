/**
 * tick-partida.ts — funciones de servidor (autoridad única del motor).
 *
...
async function aplicarSucesion(
  ...
  if (candidatos.length === 0) return partida.admin_device_id; // nadie más conectado
  ...
  // Reflejar rol visual en jugadores (display).
  await supabase
    .from("jugadores_online")
    .update({ rol: "jugador" })
 ...
 * Aplica las que corresponden sobre el MatchState y limpia esas subs de la
 * cola de cada jugador (escribe subs_pendientes actualizado).
 */
  ...
  for (const jugador of jugadores) {
    ...
    for (const sub of subs) {
      if (sub.estado === "aplicada") {
        cambio = true;
        continue; // descartar ya aplicadas
      }
      ...
      if (aprobada) {
        const ok = substitute(state, equipoIdx, sub.outId, sub.inId);
        cambio = true;
        if (!ok) {
          // no se pudo aplicar (sin cambios disponibles, jugador inválido): descartar
        }
      } else {
        restantes.push(sub); // sigue esperando consenso
      }
    }
...
export const tickPartida = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // 1. Sucesión de admin si el admin cayó.
    await aplicarSucesion(partida, jugadores);

    if (!partida.match_state) {
      return { ok: false as const, finished: false, eventos_nuevos: [] as MatchEvent[], error: "match_state no inicializado" };
    }

    // 2. Deserializar estado.
...
// ─── confirmarSub ─────────────────────────────────────────────────────────────
// Agrega una sub a la cola del jugador, o si ya existe (misma out/in) agrega el
// voto de consenso del jugador que confirma. El tick la aplicará.
...
export const confirmarSub = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // La sub se registra en la fila del jugador que la propone.
